...
**2. The 6.4% error rate destroys the precision of system comparisons.** The net adjustment being small does not mean the errors don't matter. With 99 out of 1,540 ground truth answers wrong, claiming a system scores "92.32%" to two decimal places is misleading -- the answer key itself isn't reliable enough to support that precision. When comparing two systems at 91% vs 92%, a 1-point gap is swallowed by the noise from corrupted ground truth. Small differences between systems on this benchmark are not meaningful. This problem is amplified at the per-category level, where sample sizes range from 96 to 841 and confidence intervals make the majority of adjacent-system comparisons statistically indistinguishable. See [STATISTICAL_VALIDITY.md](STATISTICAL_VALIDITY.md) for the full analysis.
